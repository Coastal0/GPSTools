...
corFile = pd.read_table("F:/CurtOval/ROHDATA/OMP_005/OMP_005_0001_1.cor", delim_whitespace = True, header = None)

if all(corFile[4] == 'S'):
    corFile[3] = corFile[3]*-1

lat_list = corFile[3].values.tolist()
lon_list = corFile[5].values.tolist()

out = []
for i,j in enumerate(lat_list):
    print(i)
    print(j)
    tFile = get_elevation(j, lon_list[i], sensor = False)
    out.append(tFile)
...
corFile.to_csv("F:/CurtOval/ROHDATA/OMP_005/OMP_005_0001_1_OUT.cor", index = False, header = None)
